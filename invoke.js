	...
	Fabric_Client.newDefaultKeyValueStore({ path: store_path
		...
		if (query_responses && query_responses.length == 1) {
			if (query_responses[0] instanceof Error) {
				...
				let dossier = JSON.parse(query_responses[0]);
				//dossier = dossier.result.payload;
				Patient = decrypt (dossier, key.privateKey); 
				//alert(patient.toString());
				//document.getElementById("output").innerHTML = patient;

				console.log("Response is ", Patient.toString());

				Patient.dossier = dossierModif;

				Patient = JSON.stringify(Patient);

				var encryptedData = encrypt(Patient, key.publicKey);

				var request = {
					//targets: let default to the peer assigned to the client
					chaincodeId: 'fabcar',
					fcn: 'invoke',
					args: [key.publicKey, encryptedData],
					chainId: 'mychannel',
					txId: tx_id
				};
				//INVOKE
